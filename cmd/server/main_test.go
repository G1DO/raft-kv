...
func TestParsePeers_ExcludesSelf(t *testing.T) {
	...
	addrs, hints, err := parsePeers(all, "n1")
	if err != nil {
...
func TestParsePeers_LegacyOtherNodesOnly(t *testing.T) {
	addrs, _, err := parsePeers("n2@n2:9090@n2:8080,n3@n3:9090@n3:8080", "n1")
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if len(addrs) != 2 {
...
func TestParsePeers_SoleMember(t *testing.T) {
	addrs, _, err := parsePeers("n1@n1:9090@n1:8080", "n1")
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if len(addrs) != 0 {
...
func TestParsePeers_Empty(t *testing.T) {
	addrs, hints, err := parsePeers("", "n1")
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if len(addrs) != 0 || len(hints) != 0 {
		t.Fatalf("expected empty, got addrs=%v hints=%v", addrs, hints)
	}
}

func TestParsePeers_Malformed(t *testing.T) {
	if _, _, err := parsePeers("not-a-valid-entry", "n1"); err == nil {
